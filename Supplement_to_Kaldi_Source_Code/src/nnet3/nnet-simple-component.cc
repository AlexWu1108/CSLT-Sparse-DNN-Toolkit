...
void NoOpComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                 ...
                                 CuMatrixBase<BaseFloat> *out) const {
  out->CopyFromMat(in);
  std::ofstream ofile("/work7/wangyanqing/ofile/NoOp");
  out->Write(ofile,false);
  ofile.close();
}
...
void AffineComponent::UpdateSimple(const CuMatrixBase<BaseFloat> &in_value,
  ...
  Matrix<BaseFloat> out_deriv_mat(out_deriv);
  Matrix<BaseFloat> in_value_mat(in_value);
  cblas_Xgemm(learning_rate_,kTrans,out_deriv_mat.Data(),out_deriv_mat.NumRows(),out_deriv_mat.NumCols(),out_deriv_mat.Stride(),kNoTrans,in_value_mat.Data(),in_value_mat.Stride(),1.0,MatAdded_mat.Data(),MatAdded_mat.NumRows(),MatAdded_mat.NumCols(),MatAdded_mat.Stride());
...
void AffineComponent::Backprop(const std::string &debug_info,
  ...
  // Modify
  //std::ofstream ofile;
  //ofile.open("/work7/wangyanqing/lins",std::ios::app);
  //linear_params_.Write(ofile,false);
  //ofile.close();
  
  std::ofstream ofile;
  ofile.open("/work7/wangyanqing/route.txt",std::ios::app);
  ofile<<"change mask\n";
  ...
  if (to_update != NULL) {
    ...
    if (to_update->is_gradient_)
      to_update->UpdateSimple(in_value, out_deriv, mask);
    else {  // the call below is to a virtual function that may be re-implemented
      //ofile<<"Update\n";
      //ofile.close();
      to_update->Update(debug_info, in_value, out_deriv, mask);  // by child classes.
    }
  }
}
